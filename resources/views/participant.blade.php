    ...
    @if($errors->any())
        ...
        <div class="description-container">
            <form class="description-form" action="{{ route('inscription') }}" method="POST" enctype="multipart/form-data">
              ...
              <form action="{{route('create.participant', $workshop)}}" method="GET" enctype="multipart/form-data">
                @csrf
            ...
            <div class="descrip-edit">
                ...
                <div class="text-des-edit">
                    ...
                    <span>{{$workshop->date1}}</span><x-input id="date1" class="mt-1 w-24" type="checkbox" name="date1" :value="old('date1') ?? $workshop->date1" required />
                </div>
            
                <div class="text-des-edit">
                    <x-label for="price" :value="__('')" />
                    <span>{{$workshop->date2}}</span><x-input id="date2" class="mt-1 w-24" type="checkbox" name="date2" :value="old('date2') ?? $workshop->date2" required />
                </div>
            
                <div class="text-des-edit">
                    <x-label for="price" :value="__('')" />
                    <span>{{$workshop->date3}}</span><x-input id="date3" class="mt-1 w-24" type="checkbox" name="date3" :value="old('date3') ?? $workshop->date3" required />
                </div>
            
                <div class="text-des-edit">
                    <x-label for="price" :value="__('')" />
                    <span>{{$workshop->date4}}</span><x-input id="date4" class="mt-1 w-24" type="checkbox" name="date4" :value="old('date4') ?? $workshop->date4" required />
                </div>
            </div>
